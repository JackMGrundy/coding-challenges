            ...
            if not (0 <= i < len(board) and 0 <= j < len(board[0])):
                ...
                for neighbor in neighbors:
                    ...
                    if 0 <= nI < len(board) and 0 <= nJ < len(board[0]) and board[nI][nJ] == 'M':
                        neighborBombsCount += 1
                        
                if 0 < neighborBombsCount:
                    board[i][j] = str(neighborBombsCount)
                    return
                else:
                    board[i][j] = 'B'
                    for neighbor in neighbors:
...
IMPORTANT: the recursive calls in case 2 only happen if the revealed square is to be a 'B'
...reminds me of an easier version of conways game of life
"""