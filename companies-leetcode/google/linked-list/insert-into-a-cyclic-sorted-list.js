...
var insert = function(head, insertVal) {
    ...
    while (true) {
        ...
        // val goes in "middle" of list
        if (head.val < insertVal && insertVal <= head.next.val) {
            [newNode.next, head.next] = [head.next, newNode];
            return res;
        }
        
        // val goes at "end" of list
        if ( insertVal > head.val && head.val > head.next.val ) {
            [newNode.next, head.next] = [head.next, newNode];
            return res;
        }
        
        // val goes at "beginning" of list
        if ( insertVal < head.next.val && head.next.val < head.val ) {
            [newNode.next, head.next] = [head.next, newNode];
            return res;
        }
        
        // hit the beginning of the list again
        if (res === head) {
            [newNode.next, head.next] = [head.next, newNode];
            return res;
        }
